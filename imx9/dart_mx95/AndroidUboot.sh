...
BOARD_SM_PATH=${VARISCITE_PATH}/imx-sm
BOARD_OEI_PATH=${VARISCITE_PATH}/imx-oei
...
build_pre_image()
{
	...
	make -C ${BOARD_SM_PATH} SM_CROSS_COMPILE="${SM_OEI_CROSS_COMPILE}" cfg config=dart-mx95-m7 1>/dev/null || exit 1
	make -C ${BOARD_SM_PATH} SM_CROSS_COMPILE="${SM_OEI_CROSS_COMPILE}" all config=dart-mx95-m7 1>/dev/null || exit 1
...
build_imx_uboot()
{
	...
	cp ${BOARD_OEI_PATH}/build/mx95-var-dart/ddr/oei-m33-ddr.bin ${BOARD_MKIMAGE_PATH}/oei-m33-ddr.bin
	cp ${BOARD_OEI_PATH}/build/mx95-var-dart/tcm/oei-m33-tcm.bin ${BOARD_MKIMAGE_PATH}
	...
	cp ${VARISCITE_PATH}/arm-trusted-firmware/build/`echo $2 | cut -d '-' -f1`/release/bl31.bin ${BOARD_MKIMAGE_PATH}/bl31.bin
