...
function add_imx95_menu() {
	img_list+=("dtbo-imx95-var-dart-dt8mcustomboard.img    (DART-MX95 on DT8MCustomBoard 3.x and above)")
	img_list+=("dtbo-imx95-var-dart-sonata.img             (DART-MX95 on Sonata-Board 1.1 and above)")
}
...
if [[ $soc_name == "showoptions" ]] && [[ ${#img_list[@]} > 1 ]] ; then
	...
	do
		if [[ -z "$opt" ]] ; then
			...
			if grep -q "i.MX8MM\|i.MX8MN\|i.MX8MQ\|i.MX8MP\|i.MX8QM\|i.MX8QP\|i.MX8QXP\|i.MX95" /sys/devices/soc0/soc_id; then
