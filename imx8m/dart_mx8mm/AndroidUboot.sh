...
build_imx_uboot()
{
	...
	cp ${UBOOT_OUT}/arch/arm/dts/imx8mm-var-dart-dt8mcustomboard.dtb  ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
	...
	if [ `echo $2 | rev | cut -d '-' -f1 | rev` != "dual" ]; then
		...
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${UBOOT_DTB}" print_fit_hab_lpddr_ddr4 || exit 1
	else
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${UBOOT_DTB}" flash_lpddr4_ddr4_evk_no_hdmi_dual_bootloader || exit 1
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${UBOOT_DTB}" PRINT_FIT_HAB_OFFSET=0x0 print_fit_hab_lpddr_ddr4 || exit 1
	fi
