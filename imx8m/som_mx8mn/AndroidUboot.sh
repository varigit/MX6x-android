...
build_imx_uboot()
{
	...
	if [ `echo $2 | cut -d '-' -f4` = "trusty" ]; then
		cp -v ${product_path}/tee-imx8mn.bin ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/tee.bin
	...
	if [ `echo $2 | rev | cut -d '-' -f1 | rev` != "dual" ]; then
		...
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MN dtbs="${UBOOT_DTB}" flash_ddr4_evk_no_hdmi_dual_bootloader || exit 1
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MN dtbs="${UBOOT_DTB}" PRINT_FIT_HAB_OFFSET=0x0 print_fit_hab_ddr4 || exit 1
	fi
